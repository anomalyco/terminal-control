...
A persistent daemon containing one to four embedded sessions, called panes, in a recursive split layout. A workspace has zero or one human terminal attachment; disconnecting the attachment preserves panes, while `run NAME` or `attach NAME` adopts a new terminal's geometry and theme and performs a full repaint. A private ID-only layout tree is authoritative for geometry, divider placement, directional focus, and close promotion while a flat pane collection retains process ownership. Human prefix controls and semantic CLI/MCP layout, focus, and close operations are adapters over this same model. Stable pane IDs are independent from position; agents discover position from pane geometry and can target input whether a human is attached or detached. Attachment presentation owns prefix decoding, transient notices, input-mode synchronization, inherited outer-terminal colors, and damage-based painting without exposing those mechanisms to the workspace model.
